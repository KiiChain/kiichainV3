...
printf "12345678\n" | "$HOME/go/bin/kiichaind" keys add $ORACLE_ACCOUNT --output json > "$HOME/.kiichain/config/oracle_key.json"
...
printf "12345678\n" | "$HOME/go/bin/kiichaind" tx oracle set-feeder "$ORACLE_ACCOUNT_ADDRESS" --from $VALIDATOR_ACCOUNT --fees 2000usei -b block -y --chain-id kiichain >/dev/null 2>&1
printf "12345678\n" | "$HOME/go/bin/kiichaind" tx bank send $VALIDATOR_ACCOUNT "$ORACLE_ACCOUNT_ADDRESS" --from $VALIDATOR_ACCOUNT 1000sei --fees 2000usei -b block -y >/dev/null 2>&1

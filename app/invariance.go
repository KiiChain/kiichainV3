...
func (app *App) LightInvarianceTotalSupply(cms sdk.CommitMultiStore) {
	defer metrics.MeasureSince(
		[]string{"kiichain", "lightinvariance_supply", "milliseconds"},
		time.Now().UTC(),
	)
	...
	for _, p := range balanceChangePairs {
		if len(p.Key) < 2 {
			// invalid key; ignore
			metrics.IncrCounterWithLabels([]string{"kiichain", "lightinvariance_supply", "invalid_changed_key"}, 1, []metrics.Label{
				{
					Name:  "type",
					Value: "kiichain",
				},

...
func BlockTest(a *App, bt *ethtests.BlockTest) {
	...
	for addr, genesisAccount := range a.EvmKeeper.BlockTest.Json.Pre {
		usei, wei := state.SplitUkiiWeiAmount(genesisAccount.Balance)
		seiAddr := a.EvmKeeper.GetKiiAddressOrDefault(a.GetContextForDeliverTx([]byte{}), addr)
		err := a.EvmKeeper.BankKeeper().AddCoins(a.GetContextForDeliverTx([]byte{}), seiAddr, sdk.NewCoins(sdk.NewCoin("ukii", usei)), true)
		if err != nil {
			panic(err)
		}
